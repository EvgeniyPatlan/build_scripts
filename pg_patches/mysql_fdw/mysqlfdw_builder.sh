...
install_deps() {
    ...
    if [ "x$OS" = "xrpm" ]; then
      ...
      if [ ${RHEL} = 8 ]; then
          ...
          INSTALL_LIST="lvm-toolset-7-clang llvm5.0-devel"
      ...
      INSTALL_LIST="percona-postgresql-common percona-postgresql11-devel mariadb-devel git rpm-build rpmdevtools systemd systemd-devel wget libxml2-devel openssl-devel perl perl-libxml-perl perl-DBD-Pg perl-Digest-SHA perl-IO-Socket-SSL perl-JSON-PP zlib-devel gcc make autoconf perl-ExtUtils-Embed"
      yum -y install ${INSTALL_LIST}
...
get_tar(){
    TARBALL=$1
    TARFILE=$(basename $(find $WORKDIR/$TARBALL -name 'percona-postgresql-mysql_fdw*.tar.gz' | sort | tail -n1))
    if [ -z $TARFILE ]
    then
        TARFILE=$(basename $(find $CURDIR/$TARBALL -name 'percona-postgresql-mysql_fdw*.tar.gz' | sort | tail -n1))
        if [ -z $TARFILE ]
        then
...
get_deb_sources(){
    param=$1
    echo $param
    FILE=$(basename $(find $WORKDIR/source_deb -name "percona-postgresql-mysql_fdw*.$param" | sort | tail -n1))
    if [ -z $FILE ]
    then
        FILE=$(basename $(find $CURDIR/source_deb -name "percona-postgresql-mysql_fdw*.$param" | sort | tail -n1))
        if [ -z $FILE ]
        then
...
build_srpm(){
    ...
    TARFILE=$(find . -name 'percona-postgresql-mysql_fdw*.tar.gz' | sort | tail -n1)
    ...
    rpmbuild -bs --define "_topdir ${WORKDIR}/rpmbuild" --define "dist .generic" \
        --define "version ${VERSION}" rpmbuild/SPECS/percona-postgresql-mysql_fdw.spec
...
build_rpm(){
    ...
    SRC_RPM=$(basename $(find $WORKDIR/srpm -name 'percona-postgresql-mysql_fdw*.src.rpm' | sort | tail -n1))
    if [ -z $SRC_RPM ]
    then
        SRC_RPM=$(basename $(find $CURDIR/srpm -name 'percona-postgresql-mysql_fdw*.src.rpm' | sort | tail -n1))
        if [ -z $SRC_RPM ]
        then
...
build_source_deb(){
    ...
    #
    TARFILE=$(basename $(find . -name 'percona-postgresql-mysql_fdw*.tar.gz' | sort | tail -n1))
    ...
    cd debian
    rm -rf changelog
    echo "percona-postgresql-mysql_fdw (${VERSION}-${RELEASE}) unstable; urgency=low" >> changelog
