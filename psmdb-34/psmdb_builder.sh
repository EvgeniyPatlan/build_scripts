...
get_sources(){
    ...
    echo "PRODUCT=${PRODUCT}" > percona-server-mongodb-34.properties
    PRODUCT_FULL=${PRODUCT}-${PSM_VER}-${PSM_RELEASE}
    echo "PRODUCT_FULL=${PRODUCT_FULL}" >> percona-server-mongodb-34.properties
    echo "VERSION=${PSM_VER}" >> percona-server-mongodb-34.properties
    echo "RELEASE=$PSM_RELEASE" >> percona-server-mongodb-34.properties
    echo "PSM_BRANCH=${PSM_BRANCH}" >> percona-server-mongodb-34.properties
    echo "JEMALLOC_TAG=${JEMALLOC_TAG}" >> percona-server-mongodb-34.properties
    echo "MONGO_TOOLS_TAG=${MONGO_TOOLS_TAG}" >> percona-server-mongodb-34.properties
    echo "BUILD_NUMBER=${BUILD_NUMBER}" >> percona-server-mongodb-34.properties
    echo "BUILD_ID=${BUILD_ID}" >> percona-server-mongodb-34.properties
    git clone "$REPO"
    retval=$?
    if [ $retval != 0 ]
    then
    ...
    echo "}" >> version.json
    #
    echo "REVISION=${REVISION}" >> ${WORKDIR}/percona-server-mongodb-34.properties
    rm -fr debian rpm
    ...
    cd ${WORKDIR}
    #
    #source ${WORKDIR}/percona-server-mongodb-34.properties
    #
    source percona-server-mongodb-34.properties
    #
    ...
    echo "UPLOAD=UPLOAD/experimental/BUILDS/${PRODUCT}-3.4/${PRODUCT}-${PSM_VER}-${PSM_RELEASE}/${PSM_BRANCH}/${REVISION}/${BUILD_ID}" >> percona-server-mongodb-34.properties
...
build_rpm(){
    ...
    export CXX=/usr/local/gcc-5.4.0/bin/g++-5.4
    #
    echo "RHEL=${RHEL}" >> percona-server-mongodb-34.properties
    echo "ARCH=${ARCH}" >> percona-server-mongodb-34.properties
    #
    file /usr/bin/scons
    #
    #if [ "x${RHEL}" == "x6" ]; then
...
build_source_deb(){
    ...
    #
    mv ${BUILDDIR}/debian/mongod.default ${BUILDDIR}/debian/percona-server-mongodb-34-server.mongod.default
    mv ${BUILDDIR}/debian/mongod.service ${BUILDDIR}/debian/percona-server-mongodb-34-server.mongod.service
    #
    mv ${TARFILE} ${PRODUCT}-34_${VERSION}.orig.tar.gz
    cd ${BUILDDIR}
...
build_deb(){
    ...
    export ARCH=$(echo $(uname -m) | sed -e 's:i686:i386:g')
    #
    echo "DEBIAN=${DEBIAN}" >> percona-server-mongodb-34.properties
    echo "ARCH=${ARCH}" >> percona-server-mongodb-34.properties
